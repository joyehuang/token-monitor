...
不设置则保持 `/api/public/stats` 和 `/api/public/badge.svg` 关闭。

### GitHub README 徽章

公开 SVG 徽章只暴露聚合 token 总数和最近更新时间。`period` 支持
`today|week|month|allTime`，`theme` 支持 `dark|light`：
...
GitHub 图片代理可能会在 Worker 的 15 秒缓存之外继续缓存 SVG，因此 README
展示是近实时而非瞬时更新。
...
| GET    | `/api/public/badge.svg`    | 无     | `PUBLIC_STATS_ENABLED=1` 时提供公开聚合 token SVG |
| GET    | `/api/stats`               | 密钥   | 聚合统计（today / week / month / allTime） |
...
密钥是必需的。当 `TOKEN_MONITOR_SECRET` 未设置时，所有数据路由都返回 `503 secret_required`，只有 `/api/health` 和可选开启的 `/api/public/stats`、`/api/public/badge.svg` 会响应。请在部署前（或部署时）设置它。
