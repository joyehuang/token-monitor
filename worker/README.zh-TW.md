...
不設定則保持 `/api/public/stats` 和 `/api/public/badge.svg` 關閉。

### GitHub README 徽章

公開 SVG 徽章只暴露彙總 token 總數和最近更新時間。`period` 支援
`today|week|month|allTime`，`theme` 支援 `dark|light`：
...
GitHub 圖片代理可能會在 Worker 的 15 秒快取之外繼續快取 SVG，因此 README
顯示是近即時而非瞬時更新。
...
| GET    | `/api/public/badge.svg`    | 無     | `PUBLIC_STATS_ENABLED=1` 時提供公開彙總 token SVG |
| GET    | `/api/stats`               | 密鑰   | 聚合統計（today / week / month / allTime） |
...
密鑰是必需的。當 `TOKEN_MONITOR_SECRET` 未設定時，所有資料路由都回傳 `503 secret_required`，只有 `/api/health` 和可選開啟的 `/api/public/stats`、`/api/public/badge.svg` 會回應。請在部署前（或部署時）設定它。
