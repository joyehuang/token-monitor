...
2. runs four period scans using `tokscale --json --client <csv> --group-by client,session,model` (today / this week via `--since <local Monday>` / month / since `allTimeSince`) on full ticks (startup / interval / manual) — serially on purpose: concurrent scans multiply peak CPU/IO. Do not use tokscale `--week`: it is a rolling last-7-days window, while our Week is the local calendar week starting Monday (like Month is the calendar month). Watch-triggered ticks instead scan only `--today` and derive week/month/allTime **exactly** via `applyPeriodDelta()` anchored to the last full scan (every tokscale period scan costs the same full-load+filter, so the win is 4 spawns→1 (8→2 when Pi is enabled: its additional provider-grouped scan preserves source attribution; Pi numeric rows from the session scan are excluded while its session details are retained); the delta is an identity for append-only logs, NOT an estimate; stale-date anchors force a full scan);
...
4. watches the per-client data directories from `watchPathsForClients()` with chokidar (`usePolling: true, interval: 2000`) and debounces refreshes by `watchDebounceMs` (no cooldown — the product promises 3–5 s updates; mid-tick watch events re-arm the debounce timer instead of coalescing). On Windows, Codex can append to an open JSONL without advancing its NTFS `LastWriteTime`, so a narrow size poll covers today's and yesterday's Codex session buckets and feeds the same debounced today-only path. The cursor/antigravity tokscale cache dirs are deliberately *not* watched — only our own `maybeSync*` calls write them, so watching them re-triggers forever — and those syncs are gated + throttled (`SYNC_MIN_INTERVAL_MS`).
...
Read-only account quotas use private `codexAccountSources` bindings and the existing HTTP usage reader, never external-profile RPC or refresh. Only opaque account keys and safe labels enter usage metadata; Pi provider ids are sources, not identity. Keep unknown/WSL sources unlinked and never sum quota percentages across devices.

Additional Codex usage profiles are configured with `TOKEN_MONITOR_CODEX_USAGE_PROFILES` or the widget folder picker. `src/shared/codexUsageProfiles.js` reads only JSONL usage/model/time/session metadata, keeps paths in local configuration only, and hashes session identity at the parse boundary with profile scope. The default tokscale-backed `~/.codex` source remains `Personal`; extra sources default to `Work` and never participate in auth, limit probing, or transcript detail. Its cache contains no source paths, raw session ids, or transcript text. Include the profile configuration fingerprint in collector-anchor validity.

All four periods are **calendar** windows in the device's local time: today, the week starting Monday (ISO-8601), the current month, and everything since `allTimeSince`. `computePeriodWindows()` keys the week by that Monday with `endsAt` at the next one, and `configFingerprint()` carries a week-semantics marker so anchors written under an older Week definition are discarded on upgrade — bump it whenever the window changes. `week` is a fork-only period: an upstream hub (`PERIODS = ['today','month','allTime']`) drops it on ingest, so a mixed fleet shows an empty WEEK until the hub itself runs this fork; `fillMissingWeek()` in `usage.js` covers the reverse case (an upstream *device* posting to our hub) by rebuilding the week from that device's daily history.
...
1. **`.env` at project root** — read by `loadDotEnv()` in `src/shared/config.js` at the top of every entry file. Only assigns keys that aren't already in `process.env`, so real env vars (systemd / launchd / Docker) still win. The primary user-facing surface is `TOKEN_MONITOR_HUB_URL`, `TOKEN_MONITOR_SECRET`, `TOKEN_MONITOR_DEVICE_ID`, `TOKEN_MONITOR_CLIENTS`, plus optional `TOKEN_MONITOR_CODEX_USAGE_PROFILES`; the rest (`TOKEN_MONITOR_INTERVAL_MS`, `TOKEN_MONITOR_PORT`, `TOKEN_MONITOR_STALE_AFTER_MS`, limit knobs, …) are accepted but kept out of `.env.example` to reduce noise.
