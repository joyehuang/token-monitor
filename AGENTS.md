...
2. runs four `tokscale --json --client <csv> --group-by client,model` calls (today / this week via `--since <local Monday>` / month / since `allTimeSince`) on full ticks (startup / interval / manual) — serially on purpose: concurrent scans multiply peak CPU/IO. Do not use tokscale `--week`: it is a rolling last-7-days window, while our Week is the local calendar week starting Monday (like Month is the calendar month). Watch-triggered ticks instead scan only `--today` and derive week/month/allTime **exactly** via `applyPeriodDelta()` anchored to the last full scan (every tokscale period scan costs the same full-load+filter, so the win is 4 spawns→1; the delta is an identity for append-only logs, NOT an estimate; stale-date anchors force a full scan);
